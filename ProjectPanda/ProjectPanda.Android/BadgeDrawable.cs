...
namespace ProjectPanda.Droid
{
    public class BadgeDrawable : Drawable
    {
        ...
        {
            _context = context;
            float textSize = context.Resources.GetDimension(Resource.Dimension.abc_text_size_subtitle_material_toolbar);//textsize_badge_count 
        ...
        public override void Draw(Canvas canvas)
        {
            ...
            //Postions the badge in the top right quad of the icon
            ...
            canvas.DrawText(_badgeValue.Length > 2 ? BadgeValueOverflow : _badgeValue, centerX, textY, _badgeText);
        }

        //Sets the text to display, badge display. Badge dsplays a '*'if more then 2 characters
        private void SetBadgeText(string text)
        {
            _badgeValue = text;
            //Only draw a a badge if the value isnt a zero
